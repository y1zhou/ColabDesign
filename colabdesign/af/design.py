...
class _af_design:
  ...
  def restart(self, seed=None, opt=None, weights=None,
    ...
    if self.protocol == "nanobody":
        nb_germline = self._pdb["batch"]["aatype"][self._target_len:]
        if "rm_aa" in kwargs:
            kwargs["rm_aa"] = None
        ...
        region_labels: list[str] = aligner.assign_cdr_labels(aligner.analyze_seq(germline_seq))
        framework_bias = np.zeros((self._binder_len, 20), dtype=float)
        for i, label in enumerate(region_labels):
            # CDR positions have zero bias
            if label.startswith("cdr"):
                continue
            framework_bias[i,residue_constants.restype_order[germline_seq[i]]] = 1e6

        self.set_seq(seq=nb_germline, mode="wt", bias=framework_bias)
    else:
